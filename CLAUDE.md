...
- [x] **2단계 실시간 차트 채우기 — Ready 파드 시리즈 · 샘플 메트릭 계약 교정 · 단독 실험 관측 트래픽** (09/06, `docs/superpowers/specs/2026-09-06-live-chart-fill-design.md`, pytest 263): 라이브에서 2단계 차트가 비어 있던 원인 3가지 해소. ① 실행 카드 `grid-cols-3` + **Ready 파드 계단형 차트**(`--success`, 정수 눈금 — 모든 앱에서 항상 값이 있는 유일한 시리즈, 백엔드 불변) · ② order-resilience-lab `app.py` `/metrics`를 `chaospilot_http_requests_total{service,status}` + `chaospilot_http_request_duration_seconds` 히스토그램(기본 버킷)으로 교체 — 비즈니스 경로(`/orders`·`/work`)만 계수, `PORT` env(테스트용). 🐛 구 샘플은 이름·라벨 불일치에 더해 YAML 블록 안 `"\\n"`이 백슬래시+n 두 글자가 되어 **본문이 한 줄로 붙어 Prometheus 파싱 실패**(시리즈 자체가 없던 원인). `tests/test_sample_app_metrics.py`가 서브프로세스로 실기동해 계약 검증 · ③ `services/live_traffic.py` `TrafficGenerator`(데몬 스레드, `probe_http` 0.5s 간격 ≈ 2 rps) — k3s 워처가 `wait_ready` 직후 시작, `finally`에서 ns 삭제 전 중지. 관측 대상은 `regression.observation_for_app(app)`(회귀 `take_sample`과 공용 — `observe_service` 우선, 없으면 `entry_service` 추론, path=`health_path`), 미해석이면 경고만 남기고 트래픽 없이 진행. 라이브(order-resilience-lab 재등록 후 단독 pod-failure exp 65): rps 0.6→7.7(ns 합산, 팬아웃 포함)·p95 50~90ms·오류 0%·Ready 9→8, Prometheus에 `chaospilot_http_*` 120 시리즈, R=0.686 저장(구 manifest exp 64는 트래픽 없음 → R None). Stub 브라우저(8001 별도 DB): 차트 3개 렌더·콘솔 오류 없음. ⚠️ 샘플 YAML이 바뀌었으니 **order-resilience-lab 재등록 필수**(라이브 DB는 이번에 갱신함) · nginx는 여전히 HTTP 메트릭 없음(Ready 파드 차트만) · EKS는 범위 밖.
